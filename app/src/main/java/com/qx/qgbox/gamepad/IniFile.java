...
public class IniFile {
    ...
    private void toIniFile(BufferedReader paramBufferedReader) {
        ...
        try {
            while (true) {
                ...
                if (localObject == null)
                    break;
                if (localPattern.matcher(localObject).matches()) {
                    localObject = localObject.trim();
                    localSection = new Section();
                    localSection.set(localSection.name, ((String) localObject).substring(1, ((String) localObject).length() - 1));
                    this.sections.put(localSection.name, localSection);
                } else {
                    String[] split = localObject.split("=");
                    if ((split.length == 2) && (localSection != null))
                        localSection.set(split[0], split[1]);
                }
            }
    ...
    public Object get(String paramString1, String paramString2, String paramString3) {
        Section section = sections.get(paramString1);
        if (section != null) {
             Object sectionObject = section.get(paramString2);
            if (sectionObject != null) {
                if (sectionObject.toString().trim().equals(""))
                    return paramString3;
    ...
    public void remove(String paramString1, String paramString2) {
         Section section = sections.get(paramString1);
        if (section != null)
            section.getValues().remove(paramString2);
    }
    ...
    public void set(String paramString1, String paramString2, Object paramObject) {
        Section localSection2 = this.sections.get(paramString1);
