  ...
  implements RandomAccess
{
  ...
  private static void buildTrieRecursive(long paramLong, Buffer paramBuffer, int paramInt1, List<ByteString> paramList, int paramInt2, int paramInt3, List<Integer> paramList1)
  {
    ...
    if (((ByteString)localObject).getByte(paramInt1) != localByteString2.getByte(paramInt1))
    {
      ...
      for (i = paramInt2; i < paramInt3; i = paramInt2)
      {
        ...
        paramInt2 = paramInt3;
        label447: if ((j == paramInt2) && (paramInt1 + 1 == ((ByteString)paramList.get(i)).size()))
        {
    ...
    paramLong = paramLong + intCount(paramBuffer) + 2L + i + 1L;
  ...
  public static Options of(ByteString[] paramArrayOfByteString)
  {
    ...
    Object localObject1 = new ArrayList(Arrays.asList(paramArrayOfByteString));
    ...
    while (i < ((List)localObject1).size())
    {
      localArrayList.set(Collections.binarySearch((List)localObject1, paramArrayOfByteString[i]), Integer.valueOf(i));
      i += 1;
    }
    ...
    for (i = 0; i < ((List)localObject1).size(); i = j)
    {
      ...
      while (k < ((List)localObject1).size())
      {
        ...
        {
          paramArrayOfByteString = new StringBuilder();
          paramArrayOfByteString.append("duplicate option: ");
          paramArrayOfByteString.append(localObject2);
          throw new IllegalArgumentException(paramArrayOfByteString.toString());
        }
    ...
    if (!((Buffer)localObject2).exhausted())
      throw new AssertionError();
    return new Options((ByteString[])paramArrayOfByteString.clone(), (int[])localObject1);
  }
